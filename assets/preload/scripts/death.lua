...
function customGameOver(loading)
    if daSongName == "no-debug" then
        if loading then
            ...
            SpriteUtil.makeAnimateSprite({tag="ammarEyes", antialiasing = false})
            runHaxeCode("game.getLuaObject('ammarEyes').camera = camDead")

            loadGraphic('ammarEyes', 'ammarvoid/eyes', 75)
            ...
            SpriteUtil.makeSprite({tag="ammarMouth",image = "ammarvoid/mouth"})
...
function customStepHit()
    if step%4 == 0 and (daSongName == "google" or daSongName == "no-debug") then
    ...
    if step%16 == 0 and daSongName == "no-debug" then
        playAnim("ammarEyes", "blink", true)
