...
function onCreatePost()
   ...
   if shadersEnabled then 
      ...
      initLuaShader("bloom")
      makeLuaSprite("bloom", "", 0, -500) setSpriteShader("bloom", "bloom")
      setShaderFloatArray("bloom", "u_samples", {16, 6})
      ...
      runHaxeCode([[
         ...
         googleHUD.setFilters([blur, bloom]);
         googleCam.setFilters([blur, bloom]);
         googleWave.setFilters([blur, bloom]);
      ]])
  end
end
...
function setBloom(intensity)
   if shadersEnabled then
      ...
      setProperty('bloom.x', intensity)
      setShaderFloat("bloom", "u_brightness", intensity)
   end
end
...
function easeBloom(intensity, dur, ease)
   if shadersEnabled then
      cancelTween("bloomtween")
      bloomTween = true
      doTweenX('bloomtween', 'bloom', intensity, dur, ease)
   end
end
...
function createParticle(amount)
   if (lowQuality and getRandomBool(10)) or not lowQuality then
      for i = 1, amount do
         ...
         SpriteUtil.makeSprite({tag=tag,image=folder.."particle", y = 720, x = getRandomFloat(0, 1280), xSize = getRandomFloat(1.5, 3)})
...
function onUpdatePost(elapsed)
   ...
   if bloomTween then
      setShaderFloat("bloom", "u_brightness", getProperty('bloom.x'))
   end
end  
...
function onStepHit()
   if (curStep >= 512 and curStep < 1008) then
      ...
      if curStep % 16 ==0 then
         setBloom(0.7)
         easeBloom(0.05, crochet/1000*3, 'quadOut')
      end
   end
